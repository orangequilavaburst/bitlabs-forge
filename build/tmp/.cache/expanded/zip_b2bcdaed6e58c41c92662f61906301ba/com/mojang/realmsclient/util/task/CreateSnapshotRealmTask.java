...
public class CreateSnapshotRealmTask extends LongRunningTask {
   ...
   public void run() {
      ...
      try {
         ...
         this.generateWorldTask = new ResettingGeneratedWorldTask(this.generationInfo, realmsserver.id, RealmsResetWorldScreen.CREATE_WORLD_RESET_TASK_TITLE, () -> {
            Minecraft.getInstance().execute(() -> {
               RealmsMainScreen.play(realmsserver, this.realmsMainScreen, true);
            });
         });
         if (this.aborted()) {
            return;
