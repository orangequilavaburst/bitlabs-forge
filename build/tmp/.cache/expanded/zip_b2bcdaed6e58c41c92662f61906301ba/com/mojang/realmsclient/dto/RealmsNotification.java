...
public class RealmsNotification {
   ...
   private static RealmsNotification parse(JsonObject p_275549_) {
      ...
      if (uuid == null) {
         ...
         switch (s) {
            case "visitUrl":
               object = RealmsNotification.VisitUrl.parse(realmsnotification, p_275549_);
               break;
            case "infoPopup":
               object = RealmsNotification.InfoPopup.parse(realmsnotification, p_275549_);
               break;
            default:
               object = realmsnotification;
         }

         return (RealmsNotification)object;
      }
   }
   ...
   public static class InfoPopup extends RealmsNotification {
      ...
      public PopupScreen buildScreen(Screen p_312232_, Consumer<UUID> p_310624_) {
         ...
         if (component == null) {
            RealmsNotification.LOGGER.warn("Realms info popup had title with no available translation: {}", (Object)this.title);
            return null;
         } else {
            PopupScreen.Builder popupscreen$builder = (new PopupScreen.Builder(p_312232_, component)).setImage(this.image).setMessage(this.message.createComponent(CommonComponents.EMPTY));
