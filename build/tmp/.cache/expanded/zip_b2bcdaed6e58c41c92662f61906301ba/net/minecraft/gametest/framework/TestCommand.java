...
public class TestCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> p_127947_) {
      p_127947_.register(Commands.literal("test").then(Commands.literal("runthis").executes((p_308552_) -> {
         return runNearbyTest(p_308552_.getSource(), false);
      }).then(Commands.literal("untilFailed").executes((p_308553_) -> {
         return runNearbyTest(p_308553_.getSource(), true);
      }))).then(Commands.literal("resetthis").executes((p_308543_) -> {
         return resetNearbyTest(p_308543_.getSource());
      })).then(Commands.literal("runthese").executes((p_308551_) -> {
         return runAllNearbyTests(p_308551_.getSource(), false);
      ...
      })).then(Commands.literal("exportthese").executes((p_296368_) -> {
         return exportAllNearbyTests(p_296368_.getSource());
   ...
   private static int showPos(CommandSourceStack p_127960_, String p_127961_) throws CommandSyntaxException {
      ...
      if (optional.isEmpty()) {
         ...
         p_127960_.sendSuccess(() -> {
            return Component.literal("Position relative to " + s1 + ": ").append(component);
         }, false);
   ...
   private static int runNearbyTest(CommandSourceStack p_127951_, boolean p_311329_) {
      ...
      if (blockpos1 == null) {
         ...
         runTest(serverlevel, blockpos1, (MultipleTestTracker)null, p_311329_);
         return 1;
      }
   }

   private static int resetNearbyTest(CommandSourceStack p_310361_) {
      BlockPos blockpos = BlockPos.containing(p_310361_.getPosition());
      ServerLevel serverlevel = p_310361_.getLevel();
      BlockPos blockpos1 = StructureUtils.findNearestStructureBlock(blockpos, 15, serverlevel);
      if (blockpos1 == null) {
         say(serverlevel, "Couldn't find any structure block within 15 radius", ChatFormatting.RED);
         return 0;
   ...
   private static int runAllNearbyTests(CommandSourceStack p_128002_, boolean p_312592_) {
      ...
      if (collection.isEmpty()) {
         ...
         collection.forEach((p_308550_) -> {
            runTest(serverlevel, p_308550_, multipletesttracker, p_312592_);
         });
         return 1;
      }
   }

   private static void runTest(ServerLevel p_127930_, BlockPos p_127931_, @Nullable MultipleTestTracker p_127932_, boolean p_309720_) {
      ...
      if (optional.isEmpty()) {
         ...
         gametestinfo.setRerunUntilFailed(p_309720_);
         if (p_127932_ != null) {
   ...
   private static boolean verifyStructureExists(ServerLevel p_310841_, GameTestInfo p_312544_) {
      if (p_310841_.getStructureManager().get(new ResourceLocation(p_312544_.getStructureName())).isEmpty()) {
         say(p_310841_, "Test structure " + p_312544_.getStructureName() + " could not be found", ChatFormatting.RED);
         return false;
      } else {
         return true;
      }
   ...
   private static int clearAllTests(CommandSourceStack p_127953_, int p_127954_) {
      ...
      GameTestRunner.clearMarkers(serverlevel);
      BlockPos blockpos = BlockPos.containing(p_127953_.getPosition().x, (double)p_127953_.getLevel().getHeightmapPos(Heightmap.Types.WORLD_SURFACE, BlockPos.containing(p_127953_.getPosition())).getY(), p_127953_.getPosition().z);
   ...
   private static int exportAllNearbyTests(CommandSourceStack p_298362_) {
      BlockPos blockpos = BlockPos.containing(p_298362_.getPosition());
      ServerLevel serverlevel = p_298362_.getLevel();
      Collection<BlockPos> collection = StructureUtils.findStructureBlocks(blockpos, 200, serverlevel);
      if (collection.isEmpty()) {
         say(serverlevel, "Couldn't find any structure blocks within 200 block radius", ChatFormatting.RED);
         return 1;
         ...
         for(BlockPos blockpos1 : collection) {
            StructureBlockEntity structureblockentity = (StructureBlockEntity)serverlevel.getBlockEntity(blockpos1);
            if (saveAndExportTestStructure(p_298362_, structureblockentity) != 0) {
               flag = false;
            }
         }
   ...
   private static int saveAndExportTestStructure(CommandSourceStack p_309467_, StructureBlockEntity p_310131_) {
   ...
   private static void say(ServerLevel p_127934_, String p_127935_, ChatFormatting p_127936_) {
      p_127934_.getPlayers((p_127945_) -> {
         ...
         p_308546_.sendSystemMessage(Component.literal(p_127935_).withStyle(p_127936_));
      });
   }
