...
public class StatsCounterFix extends DataFix {
   ...
   private TypeRewriteRule makeStatFixer() {
      ...
      return this.fixTypeEverywhereTyped("StatsCounterFix", type, type1, (p_296638_) -> {
         ...
         if (optional.isPresent()) {
            for(Map.Entry<? extends Dynamic<?>, ? extends Dynamic<?>> entry : optional.get().entrySet()) {
               if (entry.getValue().asNumber().result().isPresent()) {
                  ...
                  if (statscounterfix$stattype != null) {
                     ...
                     map.put(dynamic1, dynamic2.set(statscounterfix$stattype.typeKey(), entry.getValue()));
                  }
               }
            }
         }
   ...
   private TypeRewriteRule makeObjectiveFixer() {
      ...
      return this.fixTypeEverywhereTyped("ObjectiveStatFix", type, type1, (p_308996_) -> {
         Dynamic<?> dynamic = p_308996_.get(DSL.remainderFinder());
         Dynamic<?> dynamic1 = dynamic.update("CriteriaName", (p_296639_) -> {
            return DataFixUtils.orElse(p_296639_.asString().result().map((p_296636_) -> {
               if (SPECIAL_OBJECTIVE_CRITERIA.contains(p_296636_)) {
                  ...
                  return statscounterfix$stattype == null ? "dummy" : V1451_6.packNamespacedWithDot(statscounterfix$stattype.type) + ":" + V1451_6.packNamespacedWithDot(statscounterfix$stattype.typeKey);
               }
            }).map(p_296639_::createString), p_296639_);
         });
