...
public class BlockEntitySignDoubleSidedEditableTextFix extends NamedEntityFix {
   ...
   private static <T> Dynamic<T> fixFrontTextTag(Dynamic<T> p_300654_) {
      ...
      List<Dynamic<T>> list = getLines(p_300654_, "Text").map((p_297945_) -> {
         return p_297945_.orElse(dynamic);
      }).toList();
      Dynamic<T> dynamic1 = p_300654_.emptyMap().set("messages", p_300654_.createList(list.stream())).set("color", p_300654_.get("Color").result().orElse(p_300654_.createString("black"))).set("has_glowing_text", p_300654_.get("GlowingText").result().orElse(p_300654_.createBoolean(false))).set("_filtered_correct", p_300654_.createBoolean(true));
   ...
   private static <T> Stream<Optional<Dynamic<T>>> getLines(Dynamic<T> p_298173_, String p_299789_) {
      return Stream.of(p_298173_.get(p_299789_ + "1").result(), p_298173_.get(p_299789_ + "2").result(), p_298173_.get(p_299789_ + "3").result(), p_298173_.get(p_299789_ + "4").result());
   }

   private static <T> Dynamic<T> createDefaultText(Dynamic<T> p_299439_) {
      return p_299439_.emptyMap().set("messages", createEmptyLines(p_299439_)).set("color", p_299439_.createString("black")).set("has_glowing_text", p_299439_.createBoolean(false));
   }
