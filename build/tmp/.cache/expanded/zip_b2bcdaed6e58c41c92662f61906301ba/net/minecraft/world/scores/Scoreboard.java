...
public class Scoreboard {
   ...
   public Objective addObjective(String p_83437_, ObjectiveCriteria p_83438_, Component p_83439_, ObjectiveCriteria.RenderType p_83440_, boolean p_311367_, @Nullable NumberFormat p_311959_) {
      if (this.objectivesByName.containsKey(p_83437_)) {
         ...
         this.objectivesByCriteria.computeIfAbsent(p_83438_, (p_310953_) -> {
   ...
   public final void forAllObjectives(ObjectiveCriteria p_83428_, ScoreHolder p_310719_, Consumer<ScoreAccess> p_83430_) {
      this.objectivesByCriteria.getOrDefault(p_83428_, Collections.emptyList()).forEach((p_309370_) -> {
         p_83430_.accept(this.getOrCreatePlayerScore(p_310719_, p_309370_, true));
      });
   }

   private PlayerScores getOrCreatePlayerInfo(String p_311117_) {
      return this.playerScores.computeIfAbsent(p_311117_, (p_309376_) -> {
   ...
   public ScoreAccess getOrCreatePlayerScore(final ScoreHolder p_310827_, final Objective p_312875_, boolean p_310024_) {
      ...
      final Score score = playerscores.getOrCreate(p_312875_, (p_309375_) -> {
