...
public class PiglinAi {
   ...
   private static void initCelebrateActivity(Brain<Piglin> p_34921_) {
      p_34921_.addActivityAndRemoveMemoryWhenStopped(Activity.CELEBRATE, 10, ImmutableList.of(avoidRepellent(), SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 14.0F), StartAttacking.<Piglin>create(AbstractPiglin::isAdult, PiglinAi::findNearestValidAttackTarget), BehaviorBuilder.<Piglin>triggerIf((p_34804_) -> {
   ...
   private static void initAdmireItemActivity(Brain<Piglin> p_34941_) {
      p_34941_.addActivityAndRemoveMemoryWhenStopped(Activity.ADMIRE_ITEM, 10, ImmutableList.of(GoToWantedItem.create(PiglinAi::isNotHoldingLovedItemInOffHand, 1.0F, true, 9), StopAdmiringIfItemTooFarAway.create(9), StopAdmiringIfTiredOfTryingToReachItem.create(200, 200)), MemoryModuleType.ADMIRING_ITEM);
   }
   ...
   private static void initRideHoglinActivity(Brain<Piglin> p_34974_) {
      p_34974_.addActivityAndRemoveMemoryWhenStopped(Activity.RIDE, 10, ImmutableList.of(Mount.create(0.8F), SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 8.0F), BehaviorBuilder.sequence(BehaviorBuilder.triggerIf(Entity::isPassenger), TriggerGate.triggerOneShuffled(ImmutableList.<Pair<? extends Trigger<? super LivingEntity>, Integer>>builder().addAll(createLookBehaviors()).add(Pair.of(BehaviorBuilder.triggerIf((p_258950_) -> {
         return true;
      }), 1)).build())), DismountOrSkipMounting.<Piglin>create(8, PiglinAi::wantsToStopRiding)), MemoryModuleType.RIDE_TARGET);
   }
   ...
   protected static void updateActivity(Piglin p_34899_) {
      ...
      if (activity != activity1) {
         getSoundForCurrentActivity(p_34899_).ifPresent(p_34899_::playSoundEvent);
      }
   ...
   public static void angerNearbyPiglins(Player p_34874_, boolean p_34875_) {
      ...
      list.stream().filter(PiglinAi::isIdle).filter((p_34881_) -> {
         ...
         if (p_309198_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
            setAngerTargetToNearestTargetablePlayerIfFound(p_309198_, p_34874_);
         } else {
            setAngerTarget(p_309198_, p_34874_);
         }
   ...
   protected static void wasHurtBy(Piglin p_34838_, LivingEntity p_34839_) {
      if (!(p_34839_ instanceof Piglin)) {
         ...
         getAvoidTarget(p_34838_).ifPresent((p_309201_) -> {
            if (p_309201_.getType() != p_34839_.getType()) {
   ...
   private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
      ...
      return CopyMemoryWithExpiry.create((p_309196_) -> {
         return p_309196_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_309196_.level().random);
   ...
   protected static void broadcastAngerTarget(AbstractPiglin p_34896_, LivingEntity p_34897_) {
      getAdultPiglins(p_34896_).forEach((p_309194_) -> {
         if (p_34897_.getType() != EntityType.HOGLIN || p_309194_.canHunt() && ((Hoglin)p_34897_).canBeHunted()) {
            setAngerTargetIfCloserThanCurrent(p_309194_, p_34897_);
         }
      });
   }
