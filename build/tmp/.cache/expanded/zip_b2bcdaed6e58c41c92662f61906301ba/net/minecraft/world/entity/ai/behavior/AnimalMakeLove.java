...
public class AnimalMakeLove extends Behavior<Animal> {
   ...
   protected boolean canStillUse(ServerLevel p_22416_, Animal p_22417_, long p_22418_) {
      if (!this.hasBreedTargetOfRightType(p_22417_)) {
         ...
         return animal.isAlive() && p_22417_.canMate(animal) && BehaviorUtils.entityIsVisible(p_22417_.getBrain(), animal) && p_22418_ <= this.spawnChildAtTime && !p_22417_.isPanicking() && !animal.isPanicking();
      }
   }
   ...
   private Optional<? extends Animal> findValidBreedPartner(Animal p_22432_) {
      return p_22432_.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().findClosest((p_309028_) -> {
         if (p_309028_.getType() == this.partnerType && p_309028_ instanceof Animal animal) {
            if (p_22432_.canMate(animal) && !animal.isPanicking()) {
               return true;
            }
         }
