...
public final class TreeGrower {
   ...
   public static final Codec<TreeGrower> CODEC = ExtraCodecs.stringResolverCodec((p_310196_) -> {
      return p_310196_.name;
   }, GROWERS::get);
   ...
   private ResourceKey<ConfiguredFeature<?, ?>> getConfiguredFeature(RandomSource p_312729_, boolean p_311061_) {
      ...
      return p_311061_ && this.flowers.isPresent() ? this.flowers.get() : this.tree.orElse((ResourceKey<ConfiguredFeature<?, ?>>)null);
   }

   @Nullable
   private ResourceKey<ConfiguredFeature<?, ?>> getConfiguredMegaFeature(RandomSource p_309400_) {
      return this.secondaryMegaTree.isPresent() && p_309400_.nextFloat() < this.secondaryChance ? this.secondaryMegaTree.get() : this.megaTree.orElse((ResourceKey<ConfiguredFeature<?, ?>>)null);
   }
   ...
   public boolean growTree(ServerLevel p_309830_, ChunkGenerator p_311976_, BlockPos p_310327_, BlockState p_312382_, RandomSource p_309951_) {
      ...
      if (resourcekey1 == null) {
         ...
         Holder<ConfiguredFeature<?, ?>> holder1 = p_309830_.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).getHolder(resourcekey1).orElse((Holder.Reference<ConfiguredFeature<?, ?>>)null);
         if (holder1 == null) {
            return false;
         } else {
   ...
   private static boolean isTwoByTwoSapling(BlockState p_310256_, BlockGetter p_311754_, BlockPos p_312442_, int p_310725_, int p_310118_) {
      ...
      return p_311754_.getBlockState(p_312442_.offset(p_310725_, 0, p_310118_)).is(block) && p_311754_.getBlockState(p_312442_.offset(p_310725_ + 1, 0, p_310118_)).is(block) && p_311754_.getBlockState(p_312442_.offset(p_310725_, 0, p_310118_ + 1)).is(block) && p_311754_.getBlockState(p_312442_.offset(p_310725_ + 1, 0, p_310118_ + 1)).is(block);
   }

   private boolean hasFlowers(LevelAccessor p_312531_, BlockPos p_312326_) {
      for(BlockPos blockpos : BlockPos.MutableBlockPos.betweenClosed(p_312326_.below().north(2).west(2), p_312326_.above().south(2).east(2))) {
