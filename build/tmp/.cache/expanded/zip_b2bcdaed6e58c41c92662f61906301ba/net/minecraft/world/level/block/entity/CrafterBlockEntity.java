...
public class CrafterBlockEntity extends RandomizableContainerBlockEntity implements CraftingContainer {
   ...
   public boolean canPlaceItem(int p_311324_, ItemStack p_312777_) {
      if (this.containerData.get(p_311324_) == 1) {
         ...
         if (i >= itemstack.getMaxStackSize()) {
            ...
            return !this.smallerStackExist(i, itemstack, p_311324_);
         }
      }
   }
   ...
   private boolean smallerStackExist(int p_312152_, ItemStack p_309554_, int p_312872_) {
      for(int i = p_312872_ + 1; i < 9; ++i) {
         if (!this.isSlotDisabled(i)) {
            ...
            if (itemstack.isEmpty() || itemstack.getCount() < p_312152_ && ItemStack.isSameItemSameTags(itemstack, p_309554_)) {
               return true;
            }
         }
      }
   ...
   public void load(CompoundTag p_310330_) {
      ...
      int[] aint = p_310330_.getIntArray("disabled_slots");
   ...
   public boolean stillValid(Player p_311318_) {
      if (this.level != null && this.level.getBlockEntity(this.worldPosition) == this) {
         return !(p_311318_.distanceToSqr((double)this.worldPosition.getX() + 0.5D, (double)this.worldPosition.getY() + 0.5D, (double)this.worldPosition.getZ() + 0.5D) > 64.0D);
      } else {
         return false;
      }
   }
