...
public class PotionUtils {
   ...
   public static void getCustomEffects(@Nullable CompoundTag p_43569_, List<MobEffectInstance> p_43570_) {
      if (p_43569_ != null && p_43569_.contains("custom_potion_effects", 9)) {
         ListTag listtag = p_43569_.getList("custom_potion_effects", 10);
   ...
   public static void addPotionTooltip(ItemStack p_43556_, List<Component> p_43557_, float p_43558_, float p_309813_) {
      addPotionTooltip(getMobEffects(p_43556_), p_43557_, p_43558_, p_309813_);
   }

   public static void addPotionTooltip(List<MobEffectInstance> p_259687_, List<Component> p_259660_, float p_259949_, float p_309432_) {
      ...
      if (p_259687_.isEmpty()) {
         ...
         for(MobEffectInstance mobeffectinstance : p_259687_) {
            ...
            if (!mobeffectinstance.endsWithin(20)) {
               mutablecomponent = Component.translatable("potion.withDuration", mutablecomponent, MobEffectUtil.formatDuration(mobeffectinstance, p_259949_, p_309432_));
            }
      ...
      if (!list.isEmpty()) {
         ...
         for(Pair<Attribute, AttributeModifier> pair : list) {
            ...
            if (d0 > 0.0D) {
               p_259660_.add(Component.translatable("attribute.modifier.plus." + attributemodifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), Component.translatable(pair.getFirst().getDescriptionId())).withStyle(ChatFormatting.BLUE));
            } else if (d0 < 0.0D) {
               d1 *= -1.0D;
               p_259660_.add(Component.translatable("attribute.modifier.take." + attributemodifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), Component.translatable(pair.getFirst().getDescriptionId())).withStyle(ChatFormatting.RED));
            }
         }
      }
