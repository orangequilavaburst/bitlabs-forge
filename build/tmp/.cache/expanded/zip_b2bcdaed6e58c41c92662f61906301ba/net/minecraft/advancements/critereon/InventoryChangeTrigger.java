...
public class InventoryChangeTrigger extends SimpleCriterionTrigger<InventoryChangeTrigger.TriggerInstance> {
   ...
   public static record TriggerInstance(Optional<ContextAwarePredicate> player, InventoryChangeTrigger.TriggerInstance.Slots slots, List<ItemPredicate> items) implements SimpleCriterionTrigger.SimpleInstance {
      ...
      public static Criterion<InventoryChangeTrigger.TriggerInstance> hasItems(ItemPredicate.Builder... p_297239_) {
         return hasItems((ItemPredicate[])Stream.of(p_297239_).map(ItemPredicate.Builder::build).toArray((p_296132_) -> {
      ...
      public static Criterion<InventoryChangeTrigger.TriggerInstance> hasItems(ItemPredicate... p_43198_) {
         return CriteriaTriggers.INVENTORY_CHANGED.createCriterion(new InventoryChangeTrigger.TriggerInstance(Optional.empty(), InventoryChangeTrigger.TriggerInstance.Slots.ANY, List.of(p_43198_)));
      }
      ...
      public static record Slots(MinMaxBounds.Ints occupied, MinMaxBounds.Ints full, MinMaxBounds.Ints empty) {
         ...
         public boolean matches(int p_310199_, int p_310886_, int p_309638_) {
            if (!this.full.matches(p_310199_)) {
               return false;
            } else if (!this.empty.matches(p_310886_)) {
               return false;
            } else {
               return this.occupied.matches(p_309638_);
            }
         }
      }
   }
