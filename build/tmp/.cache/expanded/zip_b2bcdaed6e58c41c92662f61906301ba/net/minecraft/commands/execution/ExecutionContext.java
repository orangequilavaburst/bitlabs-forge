...
public class ExecutionContext<T> implements AutoCloseable {
   ...
   public static <T extends ExecutionCommandSource<T>> void queueInitialFunctionCall(ExecutionContext<T> p_311344_, InstantiatedFunction<T> p_309533_, T p_310187_, CommandResultCallback p_310874_) {
      p_311344_.queueNext(new CommandQueueEntry<>(createTopFrame(p_311344_, p_310874_), (new CallFunction<>(p_309533_, p_310187_.callback(), false)).bind(p_310187_)));
   }

   public static <T extends ExecutionCommandSource<T>> void queueInitialCommandExecution(ExecutionContext<T> p_311278_, String p_310967_, ContextChain<T> p_311656_, T p_312145_, CommandResultCallback p_309674_) {
   ...
   public Frame.FrameControl frameControlForDepth(int p_311323_) {
      return () -> {
         this.discardAtDepthOrHigher(p_311323_);
      };
   }
   ...
   public void runCommandQueue() {
      ...
      while(true) {
         if (this.commandQuota <= 0) {
            LOGGER.info("Command execution stopped due to limit (executed {} commands)", (int)this.commandLimit);
            break;
         }
         ...
         if (this.queueOverflow) {
            LOGGER.error("Command execution stopped due to command queue overflow (max {})", (int)10000000);
            break;
         }
