...
public class MacroFunction<T extends ExecutionCommandSource<T>> implements CommandFunction<T> {
   ...
   public InstantiatedFunction<T> instantiate(@Nullable CompoundTag p_309697_, CommandDispatcher<T> p_309980_, T p_312968_) throws FunctionInstantiationException {
      if (p_309697_ == null) {
         ...
         if (instantiatedfunction != null) {
            ...
            InstantiatedFunction<T> instantiatedfunction1 = this.substituteAndParse(this.parameters, list, p_309980_, p_312968_);
   ...
   private static String stringify(Tag p_313061_) {
      ...
      } else if (p_313061_ instanceof LongTag longtag) {
         return String.valueOf(longtag.getAsLong());
      } else {
         return p_313061_.getAsString();
      }
   }

   private static void lookupValues(List<String> p_313206_, IntList p_310595_, List<String> p_310258_) {
      p_310258_.clear();
      p_310595_.forEach((p_312583_) -> {
         p_310258_.add(p_313206_.get(p_312583_));
      });
   }

   private InstantiatedFunction<T> substituteAndParse(List<String> p_312865_, List<String> p_312778_, CommandDispatcher<T> p_311234_, T p_311575_) throws FunctionInstantiationException {
      ...
      for(MacroFunction.Entry<T> entry : this.entries) {
         ...
         list.add(entry.instantiate(list1, p_311234_, p_311575_, this.id));
      }

      return new PlainTextFunction<>(this.id().withPath((p_312634_) -> {
         return p_312634_ + "/" + p_312865_.hashCode();
      }), list);
   }
   ...
   interface Entry<T> {
      ...
      UnboundEntryAction<T> instantiate(List<String> p_312452_, CommandDispatcher<T> p_313016_, T p_309465_, ResourceLocation p_311242_) throws FunctionInstantiationException;
   }
   ...
   static class MacroEntry<T extends ExecutionCommandSource<T>> implements MacroFunction.Entry<T> {
      ...
      public MacroEntry(StringTemplate p_309563_, IntList p_312180_) {
      ...
      public UnboundEntryAction<T> instantiate(List<String> p_312101_, CommandDispatcher<T> p_309379_, T p_310478_, ResourceLocation p_312655_) throws FunctionInstantiationException {
         String s = this.template.substitute(p_312101_);

         try {
            return CommandFunction.parseCommand(p_309379_, p_310478_, new StringReader(s));
         } catch (CommandSyntaxException commandsyntaxexception) {
            throw new FunctionInstantiationException(Component.translatable("commands.function.error.parse", Component.translationArg(p_312655_), s, commandsyntaxexception.getMessage()));
         }
      }
   }
   ...
   static class PlainTextEntry<T> implements MacroFunction.Entry<T> {
      ...
      public UnboundEntryAction<T> instantiate(List<String> p_311533_, CommandDispatcher<T> p_311835_, T p_312017_, ResourceLocation p_311102_) {
