...
public class BuildContexts<T extends ExecutionCommandSource<T>> {
   @VisibleForTesting
   public static final DynamicCommandExceptionType ERROR_FORK_LIMIT_REACHED = new DynamicCommandExceptionType((p_311924_) -> {
      return Component.translatableEscape("command.forkLimit", p_311924_);
   });
   ...
   protected void execute(T p_309755_, List<T> p_310231_, ExecutionContext<T> p_311779_, Frame p_313162_, ChainModifiers p_310618_) {
      ...
      if (list.isEmpty()) {
         ...
         if (command instanceof CustomCommandExecutor) {
            ...
            ContinuationTask.schedule(p_311779_, p_313162_, list, (p_311832_, p_309437_) -> {
               return new CommandQueueEntry<>(p_311832_, executecommand.bind(p_309437_));
            });
         }
