...
public class WhitelistCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> p_139202_) {
      p_139202_.register(Commands.literal("whitelist").requires((p_139234_) -> {
         ...
         return SharedSuggestionProvider.suggest(playerlist.getPlayers().stream().filter((p_308920_) -> {
            return !playerlist.getWhiteList().isWhiteListed(p_308920_.getGameProfile());
         }).map((p_308921_) -> {
            return p_308921_.getGameProfile().getName();
         }), p_139217_);
      }).executes((p_139224_) -> {
      ...
      for(GameProfile gameprofile : p_139212_) {
         if (!userwhitelist.isWhiteListed(gameprofile)) {
            ...
            p_139211_.sendSuccess(() -> {
               return Component.translatable("commands.whitelist.add.success", Component.literal(gameprofile.getName()));
            }, true);
            ++i;
         }
      }
      ...
      for(GameProfile gameprofile : p_139222_) {
         if (userwhitelist.isWhiteListed(gameprofile)) {
            ...
            p_139221_.sendSuccess(() -> {
               return Component.translatable("commands.whitelist.remove.success", Component.literal(gameprofile.getName()));
            }, true);
            ++i;
         }
      }
