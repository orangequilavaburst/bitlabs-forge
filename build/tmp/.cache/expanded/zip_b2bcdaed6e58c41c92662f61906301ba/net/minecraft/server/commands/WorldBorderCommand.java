...
public class WorldBorderCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> p_139247_) {
      ...
      }).then(Commands.literal("add").then(Commands.argument("distance", DoubleArgumentType.doubleArg((double)-5.999997E7F, (double)5.999997E7F)).executes((p_308922_) -> {
         return setSize(p_308922_.getSource(), p_308922_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_308922_, "distance"), 0L);
      }).then(Commands.argument("time", IntegerArgumentType.integer(0)).executes((p_308923_) -> {
         return setSize(p_308923_.getSource(), p_308923_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_308923_, "distance"), p_308923_.getSource().getLevel().getWorldBorder().getLerpRemainingTime() + (long)IntegerArgumentType.getInteger(p_308923_, "time") * 1000L);
      })))).then(Commands.literal("set").then(Commands.argument("distance", DoubleArgumentType.doubleArg((double)-5.999997E7F, (double)5.999997E7F)).executes((p_139286_) -> {
   ...
   private static int setDamageBuffer(CommandSourceStack p_139257_, float p_139258_) throws CommandSyntaxException {
      ...
      if (worldborder.getDamageSafeZone() == (double)p_139258_) {
         ...
         p_139257_.sendSuccess(() -> {
            return Component.translatable("commands.worldborder.damage.buffer.success", String.format(Locale.ROOT, "%.2f", p_139258_));
         }, true);
   ...
   private static int setDamageAmount(CommandSourceStack p_139270_, float p_139271_) throws CommandSyntaxException {
      ...
      if (worldborder.getDamagePerBlock() == (double)p_139271_) {
         ...
         p_139270_.sendSuccess(() -> {
            return Component.translatable("commands.worldborder.damage.amount.success", String.format(Locale.ROOT, "%.2f", p_139271_));
         }, true);
   ...
   private static int setWarningTime(CommandSourceStack p_139260_, int p_139261_) throws CommandSyntaxException {
      ...
      if (worldborder.getWarningTime() == p_139261_) {
         ...
         p_139260_.sendSuccess(() -> {
            return Component.translatable("commands.worldborder.warning.time.success", p_139261_);
         }, true);
         return p_139261_;
      }
   }
   ...
   private static int setWarningDistance(CommandSourceStack p_139273_, int p_139274_) throws CommandSyntaxException {
      ...
      if (worldborder.getWarningBlocks() == p_139274_) {
         ...
         p_139273_.sendSuccess(() -> {
            return Component.translatable("commands.worldborder.warning.distance.success", p_139274_);
         }, true);
         return p_139274_;
      }
   }
   ...
   private static int setCenter(CommandSourceStack p_139263_, Vec2 p_139264_) throws CommandSyntaxException {
      ...
      if (worldborder.getCenterX() == (double)p_139264_.x && worldborder.getCenterZ() == (double)p_139264_.y) {
         ...
         p_139263_.sendSuccess(() -> {
            return Component.translatable("commands.worldborder.center.success", String.format(Locale.ROOT, "%.2f", p_139264_.x), String.format(Locale.ROOT, "%.2f", p_139264_.y));
         }, true);
         return 0;
      } else {
   ...
   private static int setSize(CommandSourceStack p_139253_, double p_139254_, long p_139255_) throws CommandSyntaxException {
      ...
      } else if (p_139254_ > (double)5.999997E7F) {
         ...
         if (p_139255_ > 0L) {
            ...
            if (p_139254_ > d0) {
               p_139253_.sendSuccess(() -> {
                  return Component.translatable("commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", p_139254_), Long.toString(p_139255_ / 1000L));
               }, true);
            } else {
               p_139253_.sendSuccess(() -> {
                  return Component.translatable("commands.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", p_139254_), Long.toString(p_139255_ / 1000L));
               }, true);
            }
         } else {
            worldborder.setSize(p_139254_);
            p_139253_.sendSuccess(() -> {
               return Component.translatable("commands.worldborder.set.immediate", String.format(Locale.ROOT, "%.1f", p_139254_));
            }, true);
         }
