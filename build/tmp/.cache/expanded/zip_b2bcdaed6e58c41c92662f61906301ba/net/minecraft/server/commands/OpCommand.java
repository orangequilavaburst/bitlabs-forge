...
public class OpCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> p_138080_) {
      p_138080_.register(Commands.literal("op").requires((p_138087_) -> {
         ...
         return SharedSuggestionProvider.suggest(playerlist.getPlayers().stream().filter((p_308778_) -> {
            return !playerlist.isOp(p_308778_.getGameProfile());
         }).map((p_308776_) -> {
            return p_308776_.getGameProfile().getName();
         }), p_138085_);
      }).executes((p_138082_) -> {
