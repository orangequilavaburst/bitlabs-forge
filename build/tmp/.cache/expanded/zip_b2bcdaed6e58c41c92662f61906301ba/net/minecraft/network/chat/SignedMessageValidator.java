...
public interface SignedMessageValidator {
   ...
   SignedMessageValidator REJECT_ALL = (p_308576_) -> {
      LOGGER.error("Received chat message from {}, but they have no chat session initialized and secure chat is enforced", (Object)p_308576_.sender());
      return null;
   };
   ...
   public static class KeyBased implements SignedMessageValidator {
      ...
      private boolean validateChain(PlayerChatMessage p_250412_) {
         if (p_250412_.equals(this.lastMessage)) {
            ...
            LOGGER.error("Received out-of-order chat message from {}: expected index > {} for session {}, but was {} for session {}", p_250412_.sender(), this.lastMessage.link().index(), this.lastMessage.link().sessionId(), p_250412_.link().index(), p_250412_.link().sessionId());
            return false;
         } else {
            return true;
         }
      ...
      private boolean validate(PlayerChatMessage p_297346_) {
         if (this.expired.getAsBoolean()) {
            LOGGER.error("Received message from player with expired profile public key: {}", (Object)p_297346_);
            return false;
         } else if (!p_297346_.verify(this.validator)) {
            LOGGER.error("Received message with invalid signature from {}", (Object)p_297346_.sender());
            return false;
         } else {
