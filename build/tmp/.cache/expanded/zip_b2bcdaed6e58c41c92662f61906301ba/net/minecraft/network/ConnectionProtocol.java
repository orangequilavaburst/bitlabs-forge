...
public enum ConnectionProtocol {
   ...
   private ConnectionProtocol(String p_297624_, ConnectionProtocol.ProtocolBuilder p_129581_) {
      this.id = p_297624_;
   ...
   public ConnectionProtocol.CodecData<?> codec(PacketFlow p_299620_) {
      return this.flows.get(p_299620_);
   }
   ...
   public static class CodecData<T extends PacketListener> implements BundlerInfo.Provider {
      ...
      public CodecData(ConnectionProtocol p_297463_, PacketFlow p_300073_, ConnectionProtocol.PacketSet<T> p_301410_) {
         this.protocol = p_297463_;
         this.flow = p_300073_;
         this.packetSet = p_301410_;
      }
      ...
      public int packetId(Packet<?> p_301367_) {
         return this.packetSet.getId(p_301367_.getClass());
      }
      ...
      Int2ObjectMap<Class<? extends Packet<?>>> packetsByIds() {
         ...
         this.packetSet.classToId.forEach((p_297285_, p_298833_) -> {
            int2objectmap.put(p_298833_.intValue(), p_297285_);
         });
         return int2objectmap;
      }

      @Nullable
      public Packet<?> createPacket(int p_298171_, FriendlyByteBuf p_299179_) {
         return this.packetSet.createPacket(p_298171_, p_299179_);
      }

      public boolean isValidPacketType(Packet<?> p_300854_) {
         return this.packetSet.isKnownPacket(p_300854_.getClass());
      }
   }

   static class PacketSet<T extends PacketListener> {
      private static final Logger LOGGER = LogUtils.getLogger();
      final Object2IntMap<Class<? extends Packet<? super T>>> classToId = Util.make(new Object2IntOpenHashMap<>(), (p_129613_) -> {
      ...
      public <P extends BundlePacket<T>> ConnectionProtocol.PacketSet<T> withBundlePacket(Class<P> p_265034_, Function<Iterable<Packet<T>>, P> p_265591_) {
         if (this.bundlerInfo != BundlerInfo.EMPTY) {
            ...
            this.addPacket(BundleDelimiterPacket.class, (p_264723_) -> {
      ...
      public boolean isKnownPacket(Class<?> p_298609_) {
         return this.classToId.containsKey(p_298609_) || this.extraClasses.contains(p_298609_);
      }
   ...
   static class ProtocolBuilder {
      ...
      public Map<PacketFlow, ConnectionProtocol.CodecData<?>> buildCodecs(ConnectionProtocol p_298845_) {
         ...
         for(PacketFlow packetflow : PacketFlow.values()) {
            ...
            if (packetset == null) {
               throw new IllegalStateException("Missing packets for flow " + packetflow + " in protocol " + p_298845_);
            }

            map.put(packetflow, new ConnectionProtocol.CodecData<>(p_298845_, packetflow, packetset));
         }
