...
public class RecoverWorldDataScreen extends Screen {
   ...
   public RecoverWorldDataScreen(Minecraft p_310416_, BooleanConsumer p_312140_, LevelStorageSource.LevelStorageAccess p_310102_) {
      ...
      Component component = Component.empty().append(this.buildInfo(p_310102_, false, exception)).append("\n").append(this.buildInfo(p_310102_, true, exception1));
      ...
      (linearlayout.addChild(Button.builder(RESTORE_BUTTON, (p_311022_) -> {
         this.attemptRestore(p_310416_);
      }).size(120, 20).tooltip(flag ? null : Tooltip.create(NO_FALLBACK_TOOLTIP)).build())).active = flag;
      this.layout.addChild(linearlayout);
      this.layout.addChild(Button.builder(CommonComponents.GUI_BACK, (p_311773_) -> {
         this.onClose();
      }).size(120, 20).build());
   ...
   private void attemptRestore(Minecraft p_311355_) {
      ...
      if (exception != null && exception1 == null) {
         p_311355_.forceSetScreen(new GenericDirtMessageScreen(Component.translatable("recover_world.restoring")));
         ...
         if (this.storageAccess.restoreLevelDataFromOld()) {
            ...
            p_311355_.setScreen(new AlertScreen(() -> {
               this.callback.accept(false);
            }, DONE_TITLE, DONE_FAILED));
         }

      } else {
         LOGGER.error("Failed to recover world, files not as expected. level.dat: {}, level.dat_old: {}", exception != null ? exception.getMessage() : "no issues", exception1 != null ? exception1.getMessage() : "no issues");
         p_311355_.setScreen(new AlertScreen(() -> {
            this.callback.accept(false);
         }, DONE_TITLE, DONE_FAILED));
      }
   ...
   private Component buildInfo(LevelStorageSource.LevelStorageAccess p_311955_, boolean p_311169_, @Nullable Exception p_312117_) {
      if (p_311169_ && p_312117_ instanceof FileNotFoundException) {
         ...
         MutableComponent mutablecomponent1 = instant != null ? Component.literal(WorldSelectionList.DATE_FORMAT.format(instant)) : Component.translatable("recover_world.state_entry.unknown");
