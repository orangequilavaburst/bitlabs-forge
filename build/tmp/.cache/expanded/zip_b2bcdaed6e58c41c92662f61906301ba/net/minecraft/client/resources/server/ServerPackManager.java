...
public class ServerPackManager {
   ...
   public ServerPackManager(PackDownloader p_313039_, PackLoadFeedback p_311463_, PackReloadConfig p_312595_, Runnable p_310909_, ServerPackManager.PackPromptStatus p_311512_) {
   ...
   public void pushLocalPack(UUID p_312688_, Path p_312014_) {
      if (this.packPromptStatus == ServerPackManager.PackPromptStatus.DECLINED) {
         ...
         ServerPackManager.ServerPackData serverpackmanager$serverpackdata = new ServerPackManager.ServerPackData(p_312688_, url, (HashCode)null);
   ...
   private boolean updateDownloads() {
      ...
      if (!list.isEmpty()) {
         ...
         this.downloader.download(map, (p_310750_) -> {
            this.onDownload(list, p_310750_);
         });
      }
   ...
   private void triggerReloadIfNeeded() {
      ...
      for(ServerPackManager.ServerPackData serverpackmanager$serverpackdata : this.packs) {
         ...
         boolean flag1 = serverpackmanager$serverpackdata.promptAccepted && serverpackmanager$serverpackdata.downloadStatus == ServerPackManager.PackDownloadStatus.DONE && !serverpackmanager$serverpackdata.isRemoved();
      ...
      if (flag) {
         ...
         this.reloadConfig.scheduleReload(new PackReloadConfig.Callbacks() {
            ...
            public List<PackReloadConfig.IdAndPath> packsToLoad() {
               return list.stream().map((p_312955_) -> {
                  return new PackReloadConfig.IdAndPath(p_312955_.id, p_312955_.path);
               }).toList();
            }
         });
      }
   ...
   static enum RemovalReason {
      ...
      private RemovalReason(@Nullable PackLoadFeedback.FinalResult p_312250_) {
