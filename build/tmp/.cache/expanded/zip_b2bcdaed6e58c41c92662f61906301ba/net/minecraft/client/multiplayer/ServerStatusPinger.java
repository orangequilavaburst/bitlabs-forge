   ...
   void pingLegacyServer(InetSocketAddress p_171812_, final ServerAddress p_300887_, final ServerData p_171813_) {
      (new Bootstrap()).group(Connection.NETWORK_WORKER_GROUP.get()).handler(new ChannelInitializer<Channel>() {
         protected void initChannel(Channel p_105498_) {
            ...
            p_105498_.pipeline().addLast(new LegacyServerPinger(p_300887_, (p_298744_, p_300358_, p_297298_, p_299389_, p_297985_) -> {
               p_171813_.protocol = -1;
               p_171813_.version = Component.literal(p_300358_);
               p_171813_.motd = Component.literal(p_297298_);
               p_171813_.status = ServerStatusPinger.formatPlayerCount(p_299389_, p_297985_);
               p_171813_.players = new ServerStatus.Players(p_297985_, p_299389_, List.of());
            }));
         }
