...
public class AccountProfileKeyPairManager implements ProfileKeyPairManager {
   ...
   private ProfileKeyPair fetchProfileKeyPair(UserApiService p_253844_) throws CryptException, IOException {
      ...
      if (keypairresponse != null) {
         ...
         return new ProfileKeyPair(Crypt.stringToPemRsaPrivateKey(keypairresponse.keyPair().privateKey()), new ProfilePublicKey(profilepublickey$data), Instant.parse(keypairresponse.refreshedAfter()));
      } else {
   ...
   private static ProfilePublicKey.Data parsePublicKey(KeyPairResponse p_253834_) throws CryptException {
      KeyPairResponse.KeyPair keypair = p_253834_.keyPair();
      if (!Strings.isNullOrEmpty(keypair.publicKey()) && p_253834_.publicKeySignature() != null && p_253834_.publicKeySignature().array().length != 0) {
         try {
