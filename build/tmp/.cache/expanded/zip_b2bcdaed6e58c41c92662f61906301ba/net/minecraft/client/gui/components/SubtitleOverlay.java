      ...
      if (this.isListening) {
         ...
         if (!this.audibleSubtitles.isEmpty()) {
            ...
            while(iterator.hasNext()) {
               ...
               if ((double)subtitleoverlay$subtitle1.getTime() + 3000.0D * d0 <= (double)Util.getMillis()) {
                  iterator.remove();
               } else {
            ...
            for(SubtitleOverlay.Subtitle subtitleoverlay$subtitle2 : this.audibleSubtitles) {
               ...
               int j1 = Mth.floor(Mth.clampedLerp(255.0F, 75.0F, (float)(Util.getMillis() - subtitleoverlay$subtitle2.getTime()) / (float)(3000.0D * d0)));
               int k1 = j1 << 16 | j1 << 8 | j1;
               p_282562_.pose().pushPose();
               p_282562_.pose().translate((float)p_282562_.guiWidth() - (float)j * 1.0F - 2.0F, (float)(p_282562_.guiHeight() - 35) - (float)(i2 * (k + 1)) * 1.0F, 0.0F);
